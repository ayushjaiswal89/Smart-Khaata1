...
$("#home-clear").addEventListener("click", ()=>{
  if(confirm("सभी Home खर्च हटाएँ?")){
    state.home = []; 
    store.set("smart-khaata", state); 
    renderHome();
  }
...
$("#home-export").addEventListener("click", ()=>{
  ...
  const csv = ["date,category,note,amount"].concat(
    rows.map(r=>[r.date,r.category,quote(r.note),r.amount].join(","))
  ).join("\n");
...
function filteredHome(){
  ...
  return state.home.filter(r=>{
    ...
    const qOk = !q || r.category.toLowerCase().includes(q) ||
...
$("#rent-export").addEventListener("click", ()=>{
  ...
  const csv = ["date,tenant,month,amount,status,note"].concat(
    rows.map(r=>[r.date,quote(r.tenant),r.month,r.amount,r.status,quote(r.note)].join(","))
  ).join("\n");
...
function filteredRent(){
  ...
  return state.rent.filter(r=>{
    ...
    const qOk = !q || r.tenant.toLowerCase().includes(q) ||
                (r.note||"").toLowerCase().includes(q);
    return mOk && qOk;
  });
}
...
$("#farm-export").addEventListener("click", ()=>{
  ...
  const csv = ["date,type,crop,value,note"].concat(
    rows.map(r=>[r.date,r.type,quote(r.crop),r.value,quote(r.note)].join(","))
  ).join("\n");
...
function filteredFarm(){
  ...
  return state.farm.filter(r=>{
    ...
    const qOk = !q || r.crop.toLowerCase().includes(q) ||
                (r.note||"").toLowerCase().includes(q);
    return tOk && qOk;
  });
}
...
$("#backup-import").addEventListener("change", async (e)=>{
  ...
  try{
    ...
    alert("JSON फ़ाइल मान्य नहीं है.");
  }
  e.target.value="";
});
...
// ---------- FIXED DOWNLOAD FUNCTION (CSV fully mobile compatible) ----------
function download(filename, content, mime="text/csv") {

  // UTF-8 BOM जोड़ना (Hindi CSV perfect open)
  const BOM = "\uFEFF";
  ...
  a.href = url;
  a.download = filename;
  a.target = "_blank";   // MOBILE FIX: forces file opener
  ...
  setTimeout(() => {
    ...
    URL.revokeObjectURL(url);   // delay revoke helps on mobile
  }, 1500);
}
...
if (mobToggle && mobMenu) {
  ...
  // CLICKING MENU BUTTON → OPEN TAB + CLOSE MENU
  mobMenu.querySelectorAll("button").forEach(btn => {
    btn.addEventListener("click", () => {

      // Desktop tab button click simulation
      const desktopBtn = document.querySelector(`.tab-btn[data-tab='${btn.dataset.tab}']`);
